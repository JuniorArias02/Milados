...
export default function SidebarPedido({ abierto, onClose }) {
  ...
  return (
    <div className={`fixed top-0 right-0 w-full sm:w-96 h-full bg-white shadow-2xl transition-all duration-300 z-50 ${abierto ? "translate-x-0" : "translate-x-full"}`}>
      {/* Encabezado */}
      <div className="bg-gradient-to-r bg-gradient-to-br from-[#FF73D9] to-[#83CCDA] p-5 flex justify-between items-center">
        <h2 className="text-2xl font-bold text-white pacifico-regular">Tu Pedido 🍦</h2>
        <button onClick={onClose} className="text-white hover:text-[#fff9e6] transition-colors text-xl">
          &times;
        </button>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-[#5d9cec]/20 p-4 shadow-inner">
        ...
        <button
          ...
          className={`w-full py-3 rounded-xl font-bold text-white transition-colors ${pedido.length === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#ff6b6b] hover:bg-[#e55a5a]'}`}
        >
          Confirmar Pedido
        </button>
      </div>
